            ...
            <div className="grid grid-cols-1 gap-2">
              {certificateTypeOptions.map((option) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    const nextType = option.value as CertificateType;
                    setCertificateType(nextType);
                    const nextGenerated = certificateTemplates[nextType].template(formData);
                    const availableTabs = Object.keys(nextGenerated);
