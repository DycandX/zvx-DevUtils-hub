  ...
  const convertToJson = () => {
    ...
    try {
      Papa.parse(csvInput, {
        ...
        ...(options.trimHeaders && { transformHeader: (header: string) => header.trim() }),
        complete: (results) => {
          ...
          const columns = rows > 0 && jsonData[0] !== null && typeof jsonData[0] === 'object' ? Object.keys(jsonData[0]).length : 0;
