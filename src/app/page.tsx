...
const TerminalUI = dynamic(() => import("@/components/TerminalUI"), {
  ssr: false,
});
...
function TerminalSkeleton() {
  return (
    <div className="w-full rounded-xl border border-white/5 overflow-hidden flex flex-col h-[520px] bg-zinc-950/45">
      <div className="h-10 bg-zinc-900/80 border-b border-white/5" />
...
function DevUtilsSkeleton() {
  return (
    <div className="w-full rounded-xl border border-white/10 overflow-hidden flex flex-col md:flex-row h-[700px] bg-zinc-950/65">
      <div className="w-full md:w-72 bg-zinc-900/90 border-b md:border-b-0 md:border-r border-white/5 p-4">
        <div className="h-5 w-3/4 bg-zinc-800/50 rounded animate-pulse mb-6" />
      ...
      <div className="flex-1 bg-[#181825]/95" />
    </div>
  );
}
    ...
    <div className="min-h-screen bg-zinc-950 bg-dot-grid text-zinc-100 flex flex-col selection:bg-emerald-500/30 selection:text-emerald-400">
      ...
      {isAdmin && (
        <div className="bg-emerald-950/40 border-b border-emerald-500/30 text-emerald-400 py-2.5 px-4 text-xs font-mono text-center flex items-center justify-center gap-2">
          <ShieldAlert size={14} />
          <span>
            ADMINISTRATOR SESSION CONSOLE ACTIVE. PRIVILEGED ACCESS GRANTED.
          </span>
        </div>
      )}
      ...
      <main className="flex-1 max-w-[1400px] mx-auto w-full px-3 sm:px-4 lg:px-6 py-10 sm:py-16 space-y-16">
        <section className="space-y-6 max-w-4xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-md bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-mono tracking-wider font-bold">
            ...
            <span>zvx.devutils // developer swiss army knife</span>
          </div>
          ...
          <p className="text-zinc-400 text-base sm:text-lg leading-relaxed font-mono">
            A comprehensive, client-side developer swiss army knife
            consolidating essential browser tools—from formatting, validating,
            and token decoding to diff checking, hashing, and batch UUID
            generation. Built to run entirely offline inside your browser tab,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-3 gap-8 pb-8 border-b border-white/5">
          <div className="space-y-3">
            ...
            <p className="text-zinc-500 leading-relaxed text-xs sm:text-sm">
              Consolidating mission-critical local client utilities and
              technical playgrounds for the developer community. No telemetry
              tracking, zero cookies, 100% browser-contained execution.
            </p>
          </div>
          <div className="space-y-3">
            <span className="font-bold text-zinc-400 tracking-wider text-xs block uppercase">
              Quick Navigation
            </span>
            ...
            <ul className="space-y-2 text-xs sm:text-sm">
              ...
              <li>
                <a
                  ...
                  className="hover:text-emerald-400 transition-colors flex items-center gap-1.5"
                >
                  <Globe size={12} className="text-zinc-500" /> Main Portfolio
                  (zulvikar.is-a.dev){" "}
                  <ArrowUpRight size={10} className="opacity-50" />
                </a>
              </li>
              <li>
                <a
                  ...
                  className="hover:text-emerald-400 transition-colors flex items-center gap-1.5"
                >
                  <GithubIcon size={12} /> GitHub Profile (DycandX){" "}
                  <ArrowUpRight size={10} className="opacity-50" />
                </a>
              </li>
            </ul>
